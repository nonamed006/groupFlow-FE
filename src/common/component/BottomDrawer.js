...
//버튼 클릭했을때 함수
//cnt: 선택된 데이터 개수
//type: 모달 타입 => 1: 사용, 삭제, 선택해제 // 2: 미사용, 삭제, 선택해제 // 3: 삭제, 선택해제 // 4: 적용, 선택해제
//handler: 사용할 함수 // onDelete: 삭제 함수 => onclick에 올 함수
const BottomDrawer = ({ cnt, type, handler, onDelete, isDrawerClose }) => {
    return (
        <div className='drawerBody' style={{ diplay: cnt != null ? "hide" : "show" }}>
            <Flex style={{ marginTop: "18px" }}>
                ...
                {type == 1 ?
                    <>
                        <Button variant="outline" color="white" borderRadius="4px" marginRight="20px" onClick={handler}>
                            사용
                        </Button>
