...
export const corpSchema = object().shape({
  ...
  sort: number()
    .test(
      "len",
      "정렬값은 10자리 이내로 입력해주세요.",
      (sort) => sort && sort.toString().length <= 10
    )
  ...
  coAbb: string()
    .nullable()
    .max(20, "회사약칭은 20자 이내로 입력해주세요.")
    .typeError("올바른 회사약칭을 입력해주세요."),
  bsnsNum: string()
    .nullable()
    .max(20, "사업자번호는 20자 이내로 입력해주세요.")
    .typeError("올바른 사업자번호를 입력해주세요."),
  coNum: string()
    .nullable()
    .max(20, "법인번호는 20자 이내로 입력해주세요.")
    .typeError("올바른 법인번호를 입력해주세요."),
  bsType: string()
    .nullable()
    .max(20, "업태는 20자 이내로 입력해주세요.")
    .typeError("올바른 업태를 입력해주세요."),
  bsStock: string()
    .nullable()
    .max(20, "업종은 20자 이내로 입력해주세요.")
    .typeError("올바른 업종을 입력해주세요."),
  pageUrl: string()
    .nullable()
    .max(50, "홈페이지 주소는 50자 이내로 입력해주세요.")
    .typeError("올바른 홈페이지 주소를 입력해주세요."),
  coDomain: string()
    .nullable()
    .max(50, "도메인은 50자 이내로 입력해주세요.")
    .typeError("올바른 도메인을 입력해주세요."),
  fax: string()
    .nullable()
    .max(20, "팩스는 20자 이내로 입력해주세요.")
    .typeError("올바른 팩스를 입력해주세요."),
  stnd: string()
    .nullable()
    .max(20, "행정표준코드는 20자 이내로 입력해주세요.")
    .typeError("올바른 행정표준코드를 입력해주세요."),
  addrDetail: string()
    .nullable()
...
export const depSchema = object().shape({
  ...
  dpAbb: string()
    .nullable()
    .max(10, "부서약칭은 10자 이내로 입력해주세요.")
    .typeError("올바른 부서약칭을 입력해주세요."),
  stnd: string()
    .nullable()
    .max(20, "표준행정코드는 20자 이내로 입력해주세요.")
    .typeError("올바른 표준행정코드를 입력해주세요."),
  reqNm: string()
    .nullable()
    .max(10, "발신인명은 10자 이내로 입력해주세요.")
    .typeError("올바른 발신인명을 입력해주세요."),
});
...
export const empSchema = object().shape({
  ...
  empNm: string()
    .required("사원 이름을 입력해주세요.")
    .max(10, "사원 이름은 20자리 이내로 입력해주세요."),
  mailId: string()
    .required("메일ID를 입력해주세요.")
  ...
  psnMail: string()
    ...
    .max(50, "개인 메일은 50자리 이하 입력해주세요"),
  payMail: string()
    .nullable()
    .email("이메일 형식을 확인해주세요.")
    .max(50, "급여 메일은 50자리 이하 입력해주세요"),
  empTel: string()
...
export const depGrpSchema = object().shape({
  ...
  joinDt: date()
    .required("입사일을 선택해주세요.")
    .typeError("입사일을 선택해주세요."),
  jobDetail: string()
    .nullable()
    .max(20, "상세업무는 20자리 이내로 입력해주세요."),
